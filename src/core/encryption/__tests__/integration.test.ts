...
test('composed processors returns idempotent result', async () => {
	...
	const encryptedBytes2 = await cipher.encrypt(new TextEncoder().encode(textSample));
	expect(new Uint32Array(encryptedBytes)).toEqual(new Uint32Array(encryptedBytes2));

	// Encryption for 2 different texts must not be equal
	const encryptedBytes3 = await cipher.encrypt(
		new TextEncoder().encode('Another text'),
	);
