import { getItemByOffset } from '@utils/collections/getItemByOffset';

const createCollection = (count: number) =>
	Array.from({ length: count }, (_, i) => `item${i}`);
const collections = createCollection(10);

describe('Receive next item (positive offset)', () => {
	test('Give item 0/10 with offset 1 to receive 1/10', () => {
		expect(getItemByOffset(collections, 0, 1)).toBe(collections[1]);
	});
	test('Give item 1/10 with offset 1 to receive 2/10', () => {
		expect(getItemByOffset(collections, 1, 1)).toBe(collections[2]);
	});
	test('Give item 2/10 with offset 1 to receive 3/10', () => {
		expect(getItemByOffset(collections, 2, 1)).toBe(collections[3]);
	});
	test('Give item 5/10 with offset 1 to receive 6/10', () => {
		expect(getItemByOffset(collections, 5, 1)).toBe(collections[6]);
	});
});

describe('Receive previous item (negative offset)', () => {
	test('Give item 0/10 with offset -1 to receive 10/10', () => {
		expect(getItemByOffset(collections, 0, -1)).toBe(collections[9]);
	});
	test('Give item 9/10 with offset -1 to receive 8/10', () => {
		expect(getItemByOffset(collections, 9, -1)).toBe(collections[8]);
	});
	test('Give item 8/10 with offset -1 to receive 7/10', () => {
		expect(getItemByOffset(collections, 8, -1)).toBe(collections[7]);
	});
	test('Give item 5/10 with offset -1 to receive 4/10', () => {
		expect(getItemByOffset(collections, 5, -1)).toBe(collections[4]);
	});
});

describe('Circular navigation', () => {
	test('Give item 10/10 with offset 1 to receive 0/10', () => {
		expect(getItemByOffset(collections, 9, 1)).toBe(collections[0]);
	});
	test('Give item 0/10 with offset -1 to receive 10/10', () => {
		expect(getItemByOffset(collections, 0, -1)).toBe(collections[9]);
	});
});

describe('Large offsets', () => {
	test('Give item 0/10 with offset 3 to receive 3/10', () => {
		expect(getItemByOffset(collections, 0, 3)).toBe(collections[3]);
	});
	test('Give item 0/10 with offset -3 to receive 7/10', () => {
		expect(getItemByOffset(collections, 0, -3)).toBe(collections[7]);
	});

	test('Give item 1/10 with offset 10000 to receive 1/10', () => {
		expect(getItemByOffset(collections, 1, 1000)).toBe(collections[1]);
	});
	test('Give item 1/10 with offset -10000 to receive undefined', () => {
		expect(getItemByOffset(collections, 1, -1000)).toBe(undefined);
	});
});
